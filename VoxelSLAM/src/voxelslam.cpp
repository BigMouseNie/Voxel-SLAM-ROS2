...
class VOXEL_SLAM : public rclcpp::Node
{
public:
  ...
  rclcpp::Subscription<sensor_msgs::msg::PointCloud2>::SharedPtr sub_pcl_ros;
  rclcpp::Subscription<livox_ros_driver2::msg::CustomMsg>::SharedPtr sub_pcl_livox;

  // modify marker
  // VOXEL_SLAM(ros::NodeHandle &n)
  VOXEL_SLAM() : Node("cmn_voxel")
  {
    pub_cmap = this->create_publisher<sensor_msgs::msg::PointCloud2>("/map_cmap", rclcpp::QoS(10));
    pub_pmap = this->create_publisher<sensor_msgs::msg::PointCloud2>("/map_pmap", rclcpp::QoS(10));
    pub_scan = this->create_publisher<sensor_msgs::msg::PointCloud2>("/map_scan", rclcpp::SensorDataQoS());
    pub_init = this->create_publisher<sensor_msgs::msg::PointCloud2>("/map_init", rclcpp::QoS(10));
    pub_test = this->create_publisher<sensor_msgs::msg::PointCloud2>("/map_test", rclcpp::QoS(10));
    pub_curr_path = this->create_publisher<sensor_msgs::msg::PointCloud2>("/map_path", rclcpp::QoS(10));
    pub_prev_path = this->create_publisher<sensor_msgs::msg::PointCloud2>("/map_true", rclcpp::QoS(10));
    ...
    lid_topic = this->declare_parameter<std::string>("General/lid_topic", "/livox/lidar");
    imu_topic = this->declare_parameter<std::string>("General/imu_topic", "/livox/imu");
    bagname = this->declare_parameter<std::string>("General/bagname", "site3_handheld_4");
    savepath = this->declare_parameter<std::string>("General/save_path", "");
    feat.lidar_type = this->declare_parameter<int>("General/lidar_type", 0);
    feat.blind = this->declare_parameter<double>("General/blind", 0.1);
    feat.point_filter_num = this->declare_parameter<int>("General/point_filter_num", 3);
    vecT = this->declare_parameter<std::vector<double>>("General/extrinsic_tran", std::vector<double>());
    vecR = this->declare_parameter<std::vector<double>>("General/extrinsic_rota", std::vector<double>());
    is_save_map = this->declare_parameter<int>("General/is_save_map", 0);
    ...
    if (feat.lidar_type == LIVOX)
    {
      sub_pcl_livox = this->create_subscription<livox_ros_driver2::msg::CustomMsg>(
          lid_topic,
          rclcpp::SensorDataQoS(),
          std::bind(&pcl_handler<livox_ros_driver2::msg::CustomMsg>, std::placeholders::_1)
      );
    }
    else
    {
      sub_pcl_livox = this->create_subscription<livox_ros_driver2::msg::CustomMsg>(
          lid_topic,
          rclcpp::SensorDataQoS(),
          std::bind(&pcl_handler<sensor_msgs::msg::PointCloud2>, std::placeholders::_1)
      );
    }
    ...
    cov_gyr        = this->declare_parameter<double>("Odometry/cov_gyr", 0.1);
    cov_acc        = this->declare_parameter<double>("Odometry/cov_acc", 0.1);
    rand_walk_gyr  = this->declare_parameter<double>("Odometry/rdw_gyr", 1e-4);
    rand_walk_acc  = this->declare_parameter<double>("Odometry/rdw_acc", 1e-4);
    down_size      = this->declare_parameter<double>("Odometry/down_size", 0.1);
    dept_err       = this->declare_parameter<double>("Odometry/dept_err", 0.02);
    beam_err       = this->declare_parameter<double>("Odometry/beam_err", 0.05);
    voxel_size     = this->declare_parameter<double>("Odometry/voxel_size", 1.0);
    min_eigen_value= this->declare_parameter<double>("Odometry/min_eigen_value", 0.0025);
    degrade_bound  = this->declare_parameter<int>("Odometry/degrade_bound", 10);
    point_notime   = this->declare_parameter<int>("Odometry/point_notime", 0);
    ...
    win_size               = this->declare_parameter<int>("LocalBA/win_size", 10);
    max_layer              = this->declare_parameter<int>("LocalBA/max_layer", 2);
    cov_gyr                = this->declare_parameter<double>("LocalBA/cov_gyr", 0.1);
    cov_acc                = this->declare_parameter<double>("LocalBA/cov_acc", 0.1);
    rand_walk_gyr          = this->declare_parameter<double>("LocalBA/rdw_gyr", 1e-4);
    rand_walk_acc          = this->declare_parameter<double>("LocalBA/rdw_acc", 1e-4);
    min_ba_point           = this->declare_parameter<int>("LocalBA/min_ba_point", 20);
    plane_eigen_value_thre = this->declare_parameter<std::vector<double>>(
                                "LocalBA/plane_eigen_value_thre",
                                std::vector<double>({1, 1, 1, 1})
                            );
    imu_coef               = this->declare_parameter<double>("LocalBA/imu_coef", 1e-4);
    thread_num             = this->declare_parameter<int>("LocalBA/thread_num", 5);
  ...
  int initialization(std::deque<sensor_msgs::msg::Imu::SharedPtr> &imus, Eigen::MatrixXd &hess, LidarFactor &voxhess, PLV(3) &pwld, pcl::PointCloud<PointType>::Ptr pcl_curr)
  {
  ...
  void system_reset(std::deque<sensor_msgs::msg::Imu::SharedPtr> &imus)
  {
    for(auto iter=surf_map.begin(); iter!=surf_map.end(); iter++)
    {
      iter->second->tras_ptr(octos_release);
  ...
  void thd_odometry_localmapping()
  {
    ...
    while(rclcpp::ok())
    {
      ...
      is_finish = this->get_parameter("finish").as_bool();
      if(is_finish)
      {
        break;
      }
...
int main(int argc, char **argv)
{
  ...
  auto voxel_node = std::make_shared<VOXEL_SLAM>();
